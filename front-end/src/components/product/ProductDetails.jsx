...
useEffect(() => {
   if(isError) {
    toast.error(error?.data?.message);
      ...
      // Increase or Decrease thq Quantity //
      ...
      <div className="col-12 col-lg-5 img-fluid" id="product_image">
        ...
        <div className="row justify-content-start mt-5">
            {product?.images?.map((img)=>(
             ...
             <a role="button">
               <img
                 className={`d-block border rounded p-3 cursor-pointer ${img.public_url === activeImg? "border-warning": "" }`}
                 height="100"
                 width="100"
                 src={img?.public_url}
                 alt={img?.public_url}
                 onClick={(e) => setActiveImg(img.public_url)}
               />
             </a>
           </div>
            ))}
